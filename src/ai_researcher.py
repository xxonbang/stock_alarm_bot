...
[입력 데이터]
{collected_data}
- 입력 데이터에 있는 내용 중 주요 키워드 중심으로 실시간 WEB 검색을 통해 가장 최신의 정보를 활용하여 분석하고 결과를 반영하십시오.
...
4. 종목 코드 언급 시 종목명 필수: 종목 코드(예: "005930.KS", "NVDA", "TSLA")를 언급할 때는 반드시 종목명도 함께 언급하십시오. 웹 검색을 통해 정확한 종목명을 확인하십시오.
...
[작성 포맷 및 가이드라인]
[가이드라인]
1. 서론 금지: "안녕하세요", "월스트리트 경력...", "종합하여 분석합니다" 같은 문장은 절대 포함하지 마십시오. 바로 '🔥 금일 급등 예상 섹터'부터 시작하십시오.
2. 강조 표시 금지: 결과물에 강조를 위한 별표(**)는 절대 사용하지 마십시오.
3. 핵심 위주 서술
...
4. 지역 구분 서술 (필수): 모든 분석은 [🇰🇷 국내]와 [🌎 해외]로 명확히 구분하여 서술하십시오.
5. 가독성 최적화: 
  - 문단이 너무 길어지지 않게 하십시오.
...
[포멧]
다음 다섯 가지 섹션으로 구분하여 작성하되, 각 섹션은 요약식이 아닌 핵심내용 위주의 서술형(Paragraph) 문단으로 작성해야 합니다.
...
- 뉴스 섹션에 있는 기사(예: 현대차 로봇, CES, HBM 공급 계약 등)와 이 테마주들을 연결하여, **왜 이 테마가 떴는지 인과관계를 설명**하십시오.
