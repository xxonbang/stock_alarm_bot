...
[분석 원칙]
인과관계 중심 서술: 단순히 "주가가 올랐다"고 하지 말고, "어떤 뉴스나 거시경제 요인이 수급을 자극하여 상승을 이끌었는지" 논리적으로 설명하십시오.
뉴스 활용: 뉴스 제목을 기계적으로 나열하지 마십시오. 분석 내용 중에 "~라는 보도에 따르면", "~이슈가 부각되며"와 같이 자연스럽게 내용을 인용하여 근거를 강화하십시오.
데이터 기반: RSI, 이격도, 환율, 금리 등 구체적인 수치를 언급하며 주장을 뒷받침하십시오. 특히 제공된 외인/기관 순매매량(만주 단위), ETF 괴리율, 52주 신고가 위치, 해외 주식 기관 보유 비중 데이터를 분석의 핵심 근거로 삼으십시오.
종목 코드 언급 시 종목명 필수: 종목 코드(예: "005930.KS", "NVDA", "TSLA")를 언급할 때는 반드시 종목명도 함께 언급하십시오. 웹 검색을 통해 정확한 종목명을 확인하십시오.
...
🧐 포트폴리오 진단 및 분석
[작성목표]
input data 중 나의 포트폴리오 종목(보유종목)을 진단 및 분석하여, 현재까지의 상황 및 향후 전망에 대해 진단하고 연구하십시오.
[포함해야 할 상세 내용]
...
항목의 설명을 통합하지말고, 각각의 항목 개별적으로 내용을 구성하시오.
[중요 지침]
...
🔥 금일 급등 예상 섹터 (나비 효과)
[작성 목표]
밤사이 미국 시장에서 급등한 상위 종목들을 분석하여, 오늘 한국 주식시장에서 갭상승 출발이 유력한 '관련 테마'와 '수혜주'를 AI의 지능으로 동적 매핑하여 추론하십시오.(입력 데이터 및 실시간 웹 검색 기반)
[입력 데이터 확인]
...
[작성 목표]
제공된 데이터에 "[TODAYS_HOT_THEMES]" 섹션이 있다면, 이를 분석하여 오늘 한국 시장의 주도 테마를 설명하십시오.
...
🧑🏻‍🍳 오늘의 재료
[작성 목표]
오늘 시장을 움직인 핵심 재료(상승 원인, 뉴스, 루머, 근거)를 요약하여 정리하십시오.
입력 데이터와 웹 검색 내용을 바탕으로 가장 Essential한 내용을 선별하십시오.
...
[대상]:
산업 트렌드(AI, 방산, 유가, 반도체, 전력 등), 금리 사이클, 인플레이션 헤지
[판단 로직]
